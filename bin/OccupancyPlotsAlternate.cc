...
int OccupancyPlots (std::string const DataFileName)
{
  ...
  //////////Alex's Dipshitery//////////
	TH1D* TelesVEvs = new TH1D("Histogram1","Telescopes Hit Per Event",16,0,15);
	TCanvas* Histogram1 = new TCanvas("Histogram1","Histogram1",400,400);
	TH1D* clusvevs = new TH1D("Histogram2","Clusters Per Event",25,0,24);
	TCanvas* Histogram2 = new TCanvas("Histogram2","Histogram2",400,400);
  	TH1D* hitsvclus = new TH1D("Histogram3","Hits Per Cluster",30,0,29);
	TCanvas* Histogram3 = new TCanvas("Histogram3","Histogram3",400,400);
  // char buffer for writing names
  char BUFF[200];
  // Loop over all events in file
  for (int ientry = 0; Event.GetNextEvent() >= 0; ++ientry) {
		
	size_t check = Event.NTelescopes();
        int flag=0;
//             if(check==1){               //////Commented Lines used for various filtering, such as only events w/o triple coincidence
//	 	PLTTelescope* Teles = Event.Telescope(0);
  //              size_t alexander = Teles->NHitPlanes();
//                if(alexander >= 3){
//			PLTPlane* Plane3 = Event.Plane(0);
//			if(Plane3->NHits()<20){
//				flag=1;}
//			if(Plane3->Channel() != 11){
//				flag=1;}
//		}
//		if(alexander > 2){
  //                    flag=1;}
...
//	     }
        if(flag==0){
	TelesVEvs->Fill(check);
	size_t numclus = 0;
	for (size_t ipa = 0; ipa != Event.NPlanes(); ++ipa){
		PLTPlane* Plane2 = Event.Plane(ipa);
		numclus = numclus + Plane2->NClusters();
		for (size_t ipas = 0; ipas != Plane2->NClusters(); ++ipas){
			PLTCluster* Cluster2 = Plane2->Cluster(ipas);
//			if(Cluster2->NHits() > 20){
				hitsvclus->Fill(Cluster2->NHits());}
//		}
	}
	clusvevs->Fill(numclus);
    ...
    for (size_t ip = 0; ip != Event.NPlanes(); ++ip) {
      ...
      for (size_t icl = 0; icl != Plane->NClusters(); ++icl) {
        ...
        for (size_t ihit = 0; ihit != Cluster->NHits(); ++ihit) {
          ...
          if (hOccupancyMap.count(id) == 0) {
            ...
            // If we're making a nethere's a 1 in 3 chance we'll need a canvas for it
	  ...
	  // Fill this histogram with the given id
          if((Cluster->NHits() < 50)&&(Cluster->NHits() > 30)&&(Cluster->NHits() != 23)){
...
f->Write();
  ...
  Histogram1->cd();
  //TelesVEvs->LabelsDeflate();
  TelesVEvs->SetFillColor(40);
  TelesVEvs->SetYTitle("Events");
  TelesVEvs->SetXTitle("Number of Telescopes");
  TelesVEvs->GetYaxis()->SetTitleOffset(1.9);
  TelesVEvs->GetYaxis()->CenterTitle();
  TelesVEvs->Draw("");
  Histogram1->SaveAs(TString::Format("plots/TelVsEvs.gif"));
  
  Histogram2->cd();
  //clusvevs->LabelsDeflate();
  clusvevs->SetFillColor(40);
  clusvevs->SetYTitle("Events");
  clusvevs->SetXTitle("Number of Clusters");
  clusvevs->GetYaxis()->SetTitleOffset(1.9);
  clusvevs->GetYaxis()->CenterTitle();
  clusvevs->Draw("");
  Histogram2->SaveAs(TString::Format("plots/ClustersPerEvent.gif"));

  Histogram3->cd();
  //hitsvclus->LabelsDeflate();
  hitsvclus->SetFillColor(40);
  hitsvclus->SetYTitle("Clusters");
  hitsvclus->SetXTitle("Number of Hits");
  hitsvclus->GetYaxis()->SetTitleOffset(1.9);
  hitsvclus->GetYaxis()->CenterTitle();
  hitsvclus->Draw("");
  Histogram3->SaveAs(TString::Format("plots/HitsPerCluster.gif"));
